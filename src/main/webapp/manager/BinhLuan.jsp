...
<body>
	...
	<c:url var="urlUser" value="/manager/BinhLuan" />
	...
	<form  method="post">
		<div class="container">
			<h2>Danh sách người dùng bình luận</h2>
			...
			<c:forEach var="item" items="${list}">
					<tr>
						<td>${item.getMaBinhLuan()}</td>
						<td>${item.getNguoidungs().getTenDN()}</td>
						<td>${item.getPhims().getMaPhim()}</td>
						<td>${item.getSao()}</td>
