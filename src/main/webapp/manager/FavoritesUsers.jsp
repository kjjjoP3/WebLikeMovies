...
<body>
	...
	<c:url var="urlUser" value="/manager/FavoritesUsers" />
	...
	<!-- Thẻ Nav -->
		...
		<div class="container">
			<h2>Danh sách người dùng Theo Mã</h2>
			...
			<div class="row mt-3">
					<div class="col">
						<div class="form-inline">
							...
							<label style="padding-right: 15px; padding-bottom: 5px">Người Dúng Yêu Thích 
							<select name="UserId" id="UserId"
								class="form-control ml-3">
								<c:forEach var="item" items="${usersArrayList}">
								
									<c:if  test="${item.tenDN == recordValue}">
										<option value="${item.tenDN}" selected>${item.tenDN }</option>
									</c:if>
									<c:if  test="${item.tenDN != recordValue}">
										<option value="${item.tenDN}">${item.tenDN }</option>
									</c:if>
								</c:forEach>
							</select>
							</label >
							
							<button class="btn btn-outline-primary" type="submit" value="Report">Xuất</button>
			...
			<table class="table table-bordered">
				...
				<tbody>
			<c:forEach var="item" items="${yeuThichArrayList}">
					<tr>
						<td>${item.maYeuThich}</td>
						<td>${item.getPhim().getMaPhim()}</td>
						<td>${item.nguoidungs.tenDN}</td>
						<td>${item.ngayThich}</td>
